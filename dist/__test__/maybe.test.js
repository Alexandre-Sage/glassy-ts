...
(0, vitest_1.suite)("MaybeContainer", () => {
    (0, vitest_1.test)("new", () => {
        const maybeArray = src_1.Maybe.new([1, 2, 3, 4]);
        const maybeOfString = src_1.Maybe.new("HelloWorld");
    ...
    (0, vitest_1.suite)("Maybe", () => {
        (0, vitest_1.test)("isValue", async ({ expect }) => {
            const maybePromise = src_1.Maybe.new(await fakeMaybePromise(1));
        ...
        (0, vitest_1.test)("map", async ({ expect }) => {
            const maybePromise = src_1.Maybe.new("1");
        ...
        (0, vitest_1.test)("isNothing", async ({ expect }) => {
            const maybePromise = src_1.Maybe.new(await fakeMaybePromise(3));
            const maybeNothing = maybePromise.isNothing();
    ...
    (0, vitest_1.suite)("MaybeArray", () => {
        ...
        (0, vitest_1.test)("isValue", ({ expect }) => {
            const maybeArray = src_1.Maybe.new(arrayOfString);
        ...
        (0, vitest_1.test)("isEmpty", ({ expect }) => {
            const maybeArray = src_1.Maybe.new([]), maybeArrayUndefined = src_1.Maybe.new();
        ...
        (0, vitest_1.test)("map", ({ expect }) => {
            const maybeArray = src_1.Maybe.new(arrayOfNumberString);
        ...
        (0, vitest_1.test)("Chaining MaybeArray method", () => {
            const maybeArray = src_1.Maybe.new(arrayOfObject);
    ...
    (0, vitest_1.suite)("MaybeAsync", () => {
        (0, vitest_1.test)("async", async () => {
            const maybePromise = src_1.Maybe.new(Promise.resolve(["Hello"]));
            (0, vitest_1.expect)(maybePromise instanceof src_1.Maybe).to.be.equal(true);
            const resolved = await maybePromise.resolve();
            (0, vitest_1.expect)(resolved instanceof src_1.Maybe).to.be.equal(true);
        });
